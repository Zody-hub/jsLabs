const data = {
    ...
    printArray(str) {
        ...
        this.array.forEach(e => {
            if(str=="price"&&e[str].newUan!=undefined)
            {
                console.log(e.name + " " + e[str].newUan);
            }
            else {
...
function byReview(a, b) {
    let rawX = a.ratingRevievs.substr(0, a.ratingRevievs.indexOf(" "));
    let rawY = b.ratingRevievs.substr(0, b.ratingRevievs.indexOf(" "));

    let x = parseInt(rawX, 10);
...
function byPrice(a, b) {
    ...
    let rawX = (typeof a.price === 'object') ? a.price.newUan : a.price;
    let rawY = (typeof b.price === 'object') ? b.price.newUan : b.price;

    rawX = rawX.replace(/\s/g, '');
    rawY = rawY.replace(/\s/g, '');

    var x = parseInt(rawX.substr(0, rawX.indexOf("грн")), 10);
    var y = parseInt(rawY.substr(0, rawY.indexOf("грн")), 10);

    return x < y ? -1 : x > y ? 1 : 0;
}

const print = data.printArray;
console.log("Відсортоване за ціною");
const sortPrices = data.sortByPrice.bind(data);
sortPrices();
print.call(data, 'price');
console.log("Відсортоване за відгуками");
const sortReviews = data.sortByReview;
sortReviews.call(data);
print.apply(data, ['ratingRevievs']);
